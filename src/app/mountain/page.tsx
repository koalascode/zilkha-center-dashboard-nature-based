...
//   const [yesterdayEnergy, setYesterdayEnergy] = useState(0);
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const [mountainEnergy, setMountainEnergy] = useState<number | null>(null)
  ...
  const sliderTransitionDuration = 55
  ...
  // Then set the useStates above to be 0 starting point :)
    ...
    }, [currEnergyUsed, transitionDuration])
    ...
    const setMonthTemps = (monthNum: number) => {
        ...
        const setMonthData = (monthData: number[]) => {
            ...
            setTemp(t); setSolar(s); setHumidity(h); setWind(w); setTransitionDuration(5000)
            ...
            <div className={styles.main}>
                <div className={styles.sidecontainer}>
                    ...
                    <div>
                        <Mountain
                          energyUsed={isToday
                            ? (tdAvgEnergy < yestAvgEnergy ? tdAvgEnergy - 5000 : tdAvgEnergy)
                            : (yestAvgEnergy < tdAvgEnergy ? yestAvgEnergy - 5000 : yestAvgEnergy)}
                          maxEnergyUsage={Math.max(tdAvgEnergy, yestAvgEnergy) + 7000}
                        />
                        <div className={styles.btncontainer}>
                          <button className={styles.btn} onClick={() => setIsToday(true)}>Today</button>
                          <button className={styles.btn} style={{backgroundColor: isToday ? "#8b16ca" : "#005b8f"}} onClick={() => setIsToday(false)}>Yesterday</button>
                        </div>
                ...
                <div className={styles.predcontainer}>
                    ...
                    <Mountain energyUsed={currEnergyUsed} maxEnergyUsage={20000} updateDuration={transitionDuration}/>
                    <p className={styles.energyusep}><b>Energy Usage Prediction: {Math.round(currEnergyUsed)} Whr</b></p>
                    <p className={styles.energyusep}><b>Energy Generation Prediction: {Math.round(currEnergyGenerated)} Whr</b></p>
                    ...
                    <div className={styles.sliderscntr}>
                        <label>
                            Temp: {temp} ºF
                            <input ref={tempRef} className={styles.slider} type="range" min={0} max={100} step={.1} defaultValue={temp} onInput={(e) => { setTemp(parseFloat((e.target as HTMLInputElement).value)); if (selectedMonth !== 'Custom') setSelectedMonth('Custom'); if (transitionDuration !== sliderTransitionDuration) setTransitionDuration(sliderTransitionDuration) }} />
                        </label>
                        <label>
                            Solar: {solar} W/m^2
                            <input ref={solarRef} className={styles.slider} type="range" min={0} max={300} step={.3} defaultValue={solar} onInput={(e) => { setSolar(parseFloat((e.target as HTMLInputElement).value)); if (selectedMonth !== 'Custom') setSelectedMonth('Custom'); if (transitionDuration !== sliderTransitionDuration) setTransitionDuration(sliderTransitionDuration) }} />
                        </label>
                        <label>
                            Humidity: {humidity} %
                            <input ref={humidityRef} className={styles.slider} type="range" min={0} max={100} step={.1} defaultValue={humidity} onInput={(e) => { setHumidity(parseFloat((e.target as HTMLInputElement).value)); if (selectedMonth !== 'Custom') setSelectedMonth('Custom'); if (transitionDuration !== sliderTransitionDuration) setTransitionDuration(sliderTransitionDuration) }} />
                        </label>
                        <label>
                            Wind: {Math.round(wind * 80) / 10} mph
                            <input ref={windRef} className={styles.slider} type="range" min={0} max={6} step={.06} defaultValue={wind} onInput={(e) => { setWind(parseFloat((e.target as HTMLInputElement).value)); if (selectedMonth !== 'Custom') setSelectedMonth('Custom'); if (transitionDuration !== sliderTransitionDuration) setTransitionDuration(sliderTransitionDuration) }} />
                        </label>
                    </div>
                </div>
